...
class RAGAgent:
    ...
    def ask(self, question: str) -> dict:
        ...
        agent = Agent(
            ...
            goal='Answer questions about Nutrition using the database',
            backstory='You are an expert who has access to a database with content about the Nutrition.',
            tools=[query_tool],
            llm=llm,
        ...
        task = Task(description=f"Answer the following nutrition-related question using evidence from the nutrition database. Provide a clear, accurate explanation and cite the retrieved passages.\n\nQuestion: {question}",
                    agent=agent,
                    expected_output="A structured answer that: (1) summarizes the main nutrition facts, (2) uses retrieved database passages as evidence, and (3) provides a simple, actionable recommendation."
)
